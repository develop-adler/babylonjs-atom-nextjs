...
class AvatarController {
    ...
    private _updateCharacter(): void {
        ...
        if (
            this._joystick?.event?.type === "move" &&
            this._joystick?.data?.angle?.radian
        ) {
            this._isMoving = true;
            this._isDancing = false;
            ...
            const directionOffset: number =
                -this._joystick.data.angle?.radian + Math.PI * 0.5 + Math.PI;
            ...
            const joystickVector = this._joystick.data.vector;
    ...
    public get joystick(): JoystickInstance | undefined {
